...
const baseTimeApiURL = "https://timezone.abstractapi.com/v1/current_time/?api_key=6de43a8244544ffca5df7a103d0e1d95";
const apiHourly = "onecall?lat=6.5&lon=3.4&exclude=current,alerts,minutely,daily&appid=$weatherApiKey&units=metric";
...
class WeatherData {
  ...
  Future<dynamic> getCurrentWeatherData() async{
    ...
    try {
      ...
      final apiLatAndLongURL = "weather?lat=$deviceLat&lon=$deviceLong&appid=$weatherApiKey&units=metric";
  ...
  Future<dynamic> getCityWeatherData(String? cityName) async{
    try {
      final apiCityURL = "weather?q=$cityName&appid=$weatherApiKey&units=metric";
  ...
  Future<dynamic> getCityTime(String? cityLocation) async{
    try {
      ...
      var getTimeCityData = await _networkHelper.get('$baseTimeApiURL&$timeCityURL');
  ...
  Future<dynamic> getHourlyWeather() async{
    try{
      await _networkHelper.get("$apiBaseURL/$apiHourly").then((value) {
        var res = value['hourly'];
        List<Hourly> hourlyModel = List<Hourly>.from(res.map((res) => Hourly.fromJson(res)));
        print(hourlyModel);
      }).catchError((e){
        print(e);
      });
